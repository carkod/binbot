...
from pattern_detection import downtrend_patterns, reversal_confirmation, test_pattern_recognition

def candlestick_patterns(data, sd, close_price, open_price, value, chaikin_diff, regression, _send_msg, run_autotrade, symbol, ws, intercept, ma_100):
    """
    ...
    all_patterns = test_pattern_recognition(data)
    ...
    if reversal and chaikin_diff < 0 and float(intercept) > 5 and len(downtrend) == 0 and close_price > ma_100[len(ma_100) - 1]:
        ...
        run_autotrade(symbol, ws, test_only=True)
    pass