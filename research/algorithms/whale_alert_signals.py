...
class WhaleAlertSignals:
    ...
    def __init__(self):
        ...
        self.exclude_list = ["USDT", "BUSD", "BTC"]
    ...
    def run_bot(self) -> None:
        ...
        if transaction:
            msg = f'[{os.getenv("ENV")}] <strong>Whale alert</strong>: {transaction["transaction_type"]} of #{transaction["symbol"]} ({transaction["amount_usd"]} USD) from {transaction["from"]["owner"]} to {transaction["to"]["owner"]}\n- https://www.binance.com/en/trade/{transaction["symbol"]}USDT \n- Dashboard trade http://binbot.in/admin/bots/new{transaction["symbol"]}USDT'
