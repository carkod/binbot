...
class ControllerSchema:
    ...
    def __init__(self) -> object:
        ...
        if settings and "take_profit" in settings:
    ...
    def validate_model(self, data):
        ...
        if "update_required" in data:
            if not isinstance(data.get("update_required"), bool):
                if data.get("update_required") == "true" or data.get("update_required") == "True":
        ...
        if "balance_size_to_use" in data:
            if not isinstance(data.get("balance_size_to_use"), int):
                try:
                    ...
                    raise TypeError(f"balance_size_to_use must be a positive integer between 0 and 100")    
        ...
        if len(data) > 0:
            for item in data:
                raise TypeError(f"{item} was not found. If this is a new field, please add it to the ControllerSchema")
    ...
    def update(self, data):
        ...
        if validation:
            ...
            current_app.db.research_controller.update_one({"_id": "settings"}, {"$set": clean_data}, True)
            return True
        else:
            return False
