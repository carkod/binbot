    ...
    def retrieve_order(self, order_id):
        return type(
            ...
            (object,),
            {
                ...
                "type": AddOrderReq.TypeEnum.MARKET,
...
class DummyFuturesApiInsufficientBalance(DummyFuturesApi):
    def __init__(self):
        super().__init__()
        self._positions = [
            type("pos", (object,), {"current_qty": 68, "mark_price": 1.27})(),
...
class DummyFuturesApiEnumDetails(DummyFuturesApi):
    def retrieve_order(self, order_id):
        return type(
            "order",
            (object,),
...
def test_reverse_position_retries_second_order_after_insufficient_balance(
    monkeypatch,
):
    bot = make_mock_bot_active_model()
    bot.market_type = MarketType.FUTURES
    ...
    controller = DummyController()
    futures_api = DummyFuturesApiInsufficientBalance()
...
def test_reverse_position_normalizes_sdk_enum_values_in_second_order(monkeypatch):
    bot = make_mock_bot_active_model()
    bot.market_type = MarketType.FUTURES
    bot.position = Position.long
    bot.margin_short_reversal = True
    ...
    controller = DummyController()
    futures_api = DummyFuturesApiEnumDetails()
    ...
    assert reversed_bot.orders[0].order_type == AddOrderReq.TypeEnum.LIMIT.value
    assert reversed_bot.orders[0].order_side == AddOrderReq.SideEnum.SELL.value
    assert (
        reversed_bot.orders[0].time_in_force
