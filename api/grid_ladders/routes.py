...
from databases.crud.grid_ladder_crud import (
    close_grid_ladder,
    create_grid_ladder,
    create_grid_levels,
    get_active_grid_ladder_for_symbol,
    get_active_grid_ladders,
    get_grid_ladder,
    get_grid_ladders,
)
...
def _record(ladder) -> GridLadderRecord:
    return GridLadderRecord.model_validate(ladder)
...
def post_grid_ladder(
    ...
    if get_active_grid_ladder_for_symbol(session, payload.symbol) is not None:
        raise HTTPException(
            status_code=400,
            detail="An active grid ladder already exists for this symbol",
        )

    active_ladders = get_active_grid_ladders(session)
...
def list_grid_ladders(
    ...
    ladders = get_grid_ladders(session, limit=limit, offset=offset)
    return GridLadderListResponse(
        message="Successfully found grid ladders.",
        data=[_record(ladder) for ladder in ladders],
    )
...
def list_active_grid_ladders(
    ...
    ladders = get_active_grid_ladders(session)
    return GridLadderListResponse(
        message="Successfully found active grid ladders.",
        data=[_record(ladder) for ladder in ladders],
    )
...
def get_grid_ladder_by_id(
    ...
    ladder = get_grid_ladder(session, ladder_id)
    if ladder is None:
        raise HTTPException(status_code=404, detail="Grid ladder not found")

    return GridLadderResponse(
        message="Successfully found grid ladder.",
        data=_record(ladder),
    )
...
def close_grid_ladder_by_id(
    ...
    ladder = close_grid_ladder(session, ladder_id, status=GridLadderStatus.closed)
    if ladder is None:
        raise HTTPException(status_code=404, detail="Grid ladder not found")

    return GridLadderResponse(
        message="Successfully closed grid ladder.",
        data=_record(ladder),
    )
