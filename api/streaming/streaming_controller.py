...
class BbspreadsUpdater(BaseStreaming):
    ...
    def update_bots_parameters(self, bot: BotSchema, bb_spreads, collection_name="bots"):
        ...
        top_spread = supress_notation(abs((bb_spreads["bb_high"] - bb_spreads["bb_mid"]) / bb_spreads["bb_high"]) * 1000, 2)
        whole_spread = supress_notation(abs((bb_spreads["bb_high"] - bb_spreads["bb_low"]) / bb_spreads["bb_high"]) * 1000, 2)
        bottom_spread = supress_notation(abs((bb_spreads["bb_mid"] - bb_spreads["bb_low"]) / bb_spreads["bb_mid"]) * 1000, 2)
        ...
        if 8 > whole_spread > 3:
            ...
            bot.errors.append(f"Bot bb_spread too low, not updated: {top_spread}, {whole_spread}, {bottom_spread}")
            self.save_bot_streaming(bot)
