...
def upgrade() -> None:
    ...
    op.execute("""
    DO $$ BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'baseassets') THEN
            CREATE TYPE baseassets AS ENUM ('USDC', 'BTC', 'BNB');
        END IF;
    END$$;
    """)
    op.add_column(
    ...
    # Remove the default for future inserts if not desired
