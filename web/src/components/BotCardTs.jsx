...
const RenderTimestamps = (bot) => {
  if (bot.strategy === "long") {
    if (bot.deal?.buy_timestamp > 0 && bot.deal?.sell_timestamp > 0) {
      return (
        <>
          <TimestampComponent label="Open time" timestamp={bot.deal?.buy_timestamp} />
          <TimestampComponent label="Close time" timestamp={bot.deal?.sell_timestamp} />
        </>
      );
  ...
  if (bot.strategy === "margin_short") {
    if (
      ...
      return (
        <>
					<TimestampComponent label="Open time" timestamp={bot.deal?.margin_short_sell_timestamp} />
					<TimestampComponent label="Close time" timestamp={bot.deal?.sell_timestamp} />
        </>
      );
