...
def kst_oscillator(df, r1, r2, r3, r4, n1, n2, n3, n4, p):
    ...
    KSTsign = pd.Series(KST.rolling(window=9).mean(), name='KSTsign_' + str(p))
    KSTdiff = pd.Series(KST - KSTsign, name='KSTdiff_' + str(p))
    df = df.join(KST)
    df = df.join(KSTsign)
    df = df.join(KSTdiff)
    return df
