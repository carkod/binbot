...
class FORWARD():
    ...
    def sell_funds(self):
        ...
        ei = Exchange_Info()
        text = 'running sell_funds {} '.format(self.max_funds)
        print(text)
        logger(text)
        if sf.launch_kc_sell() or self.max_funds in ('BNBBTC', 'BTCBTC'):
            # sell_order(symbol)
            text = 'short {} @ {}'.format(self.max_funds, self.get_asset_current_price(self.max_funds))
            print(text)
            logger(text)
            # testing
            sold_asset = ei.find_quoteAsset(self.max_funds)
            # if sold_asset == 'BNB':
            #     self.max_funds = 'BNBBTC'
            # elif sold_asset == 'BTC':
            #     self.max_funds = 'BTCBTC'
    ...
    def get_asset_current_price(self, symbol):
        tp = Ticker_Price()
        print(symbol, tp.request_data(symbol))
        ...
        if long_asset:
            ...
            if long_asset_quote == highest_asset_quote:
                ...
                if highest_balance_asset == 'BTCBTC':
                    ...
                    long_asset_price = self.get_asset_current_price(
                        long_asset)  # testing algo
                    print('Buy order {} @ {}'.format(
                        long_asset, long_asset_price))
                    logger('Buy order {} @ {}'.format(
                        long_asset, long_asset_price))
                    ...
                    highest_balance_asset_price = self.get_asset_current_price(
                        highest_balance_asset)  # testing algo
                    long_asset_price = self.get_asset_current_price(
                        long_asset)  # testing algo
                    self.max_funds = long_asset
                    ...
                    if highest_balance_asset == long_asset:
                        return False
                    text = 'Sell order {} @ {} and buy order {} @ {}'.format(
                        highest_balance_asset, highest_balance_asset_price, long_asset, long_asset_price)
                    print(text)
                    logger(text)
                ...
                if highest_asset_quote == 'BTC':
                    ...
                    # buy_order(symbol)
                    symbol_price = self.get_asset_current_price(
                        symbol)  # testing algo
                    long_asset_price = self.get_asset_current_price(
                        long_asset)  # testing algo
                    text = 'Buy order {} @ {}. And buy order {} @ {}'.format(
                        symbol, symbol_price, long_asset, long_asset_price)
                    print(text)
                    logger(text)
                    ...
                    symbol_price = self.get_asset_current_price(
                        symbol)  # testing algo
                    text = 'Sell order {} @ {}'.format(symbol, symbol_price)
                    print(text)
                    logger(text)
        else:
            self.sell_funds()
    ...
    def programmer(self):
        ...
        held_asset = self.find_max_funds()
        if highest_fund != 'BTCBTC':
            print('held asset: ' + self.max_funds)
            self.sell_funds()
            # if (time_now > midnight) and (time_now < morning):
            #     schedule.every(1).minute.do(self.sell_funds)
            # else:
            #     schedule.every(1).minute.do(self.sell_funds)
        # schedule.every(1).minute.do(self.long_algo)
        self.long_algo()
        while True:
