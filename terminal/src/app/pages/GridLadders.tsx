...
const GridLaddersPage: FC = () => {
  ...
  const { data: allLadders = [], isFetching, refetch } = useGetGridLaddersQuery(
    { limit: 100, offset: 0 },
    { refetchOnFocus: true },
  );
  ...
  const [closeGridLadder, { isLoading: isClosing }] = useCloseGridLadderMutation();
  ...
  const filteredLadders = useMemo(() => {
    return sourceLadders.filter((ladder) => {
      const statusMatches = filterStatus === "all" || ladder.status === filterStatus;
      const symbolMatches = !symbolState || ladder.symbol.toLowerCase().includes(symbolState.toLowerCase());
  ...
  const summary = useMemo(() => {
    return filteredLadders.reduce(
      (acc, ladder) => {
        ...
        acc.openLevels += ladder.levels.filter((level) => level.entry_order_id || level.take_profit_order_id).length;
        return acc;
      },
      { active: 0, reserved: 0, used: 0, realized: 0, unrealized: 0, openLevels: 0 },
    );
  }, [filteredLadders]);
  ...
  const handleSelect = (id: string) => {
    setSelectedCards((prev) =>
      prev.includes(id) ? prev.filter((cardId) => cardId !== id) : prev.concat(id),
    );
  };
  ...
  const confirmClose = async (action: number) => {
    ...
    await closeGridLadder({ id: ladderToClose, reason: "manual_close" }).unwrap();
    setSelectedCards([]);
    setLadderToClose(null);
    await refetch();
  };
  ...
  return (
    <Container fluid>
      ...
      <Stack direction="horizontal" gap={2} className="mb-3 flex-wrap">
        ...
        <Badge bg="secondary">Reserved margin: {summary.reserved.toFixed(2)} USDC</Badge>
        <Badge bg="secondary">Used margin: {summary.used.toFixed(2)} USDC</Badge>
        <Badge bg={summary.realized >= 0 ? "success" : "danger"}>Realized PnL: {summary.realized.toFixed(4)}</Badge>
        <Badge bg={summary.unrealized >= 0 ? "success" : "danger"}>Unrealized PnL: {summary.unrealized.toFixed(4)}</Badge>
      ...
      <Row className="mb-3">
        ...
        <Col md={3}>
          <Button
            ...
            onClick={() => setSelectedCards(filteredLadders.map((ladder: GridLadder) => ladder.id))}
          >
            Select all
          </Button>{" "}
          <Button variant="outline-secondary" onClick={() => setSelectedCards([])}>Unselect all</Button>
        </Col>
      </Row>

      {filteredLadders.length === 0 ? (
        <p>{showActiveOnly ? "No active grid ladders" : "No grid ladders deployed yet"}</p>
      ) : (
        <Row>
      ...
      <ConfirmModal show={Boolean(ladderToClose)} handleActions={confirmClose} primary="Close ladder">
        Closing a grid ladder should cancel open grid orders and stop the ladder. If positions are open,
        backend close behavior depends on the grid manager state.
      </ConfirmModal>
    </Container>
  );
};
