...
export const blacklistApiSlice = userApiSlice.injectEndpoints({
  endpoints: (build) => ({
    ...
    deleteBlacklistItem: build.mutation<void, string>({
        query: ({ pair }) => ({
          url: `${import.meta.env.VITE_RESEARCH_BLACKLIST}/${pair}` || "/blacklist",
          method: "DELETE",
          invalidatesTags: ["blacklist"],
        }),
        transformResponse: ({ data, message, error }, meta, arg) => {
...
export const { useGetBlacklistQuery, useAddBlacklistItemMutation, useDeleteBlacklistItemMutation } = blacklistApiSlice;
